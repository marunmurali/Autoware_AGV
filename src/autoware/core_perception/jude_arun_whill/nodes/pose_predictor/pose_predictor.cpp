...
using namespace std;
  ...
  int ScoreCount=0,NDTPoseFlag=0;
  //tf2_ros::Buffer tfBuffer;
  double delta_time=.1;
  double roll, pitch, yaw_mod,vx_odo,tw_odo;
  float ndt_score_limit = 0.4;
...
int main(int argc, char** argv)
{
  ...
  if((ndt_time.toSec()-last_ndt_time.toSec())>2 && ndt_score < ndt_score_limit && ScoreCount>10)
  {
